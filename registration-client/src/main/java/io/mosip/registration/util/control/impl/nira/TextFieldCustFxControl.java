 ...
 *
 */
public class TextFieldCustFxControl extends FxControl {

	/**
	 * Instance of {@link Logger}
	 */
	private static final Logger LOGGER = AppConfig.getLogger(TextFieldCustFxControl.class);
	...
	private VBox create(UiFieldDTO uiFieldDTO) {
		...
		Label fieldTitle = getLabel(uiFieldDTO.getId() + RegistrationConstants.LABEL, "",
				RegistrationConstants.DEMOGRAPHIC_FIELD_LABEL, true, simpleTypeVBox.getWidth() - 10);
		...
		switch (this.uiFieldDTO.getType()) {
			case RegistrationConstants.SIMPLE_TYPE :
				getRegistrationDTo().getSelectedLanguagesByApplicant().forEach(langCode -> {
					...
					vBox.getChildren().add(createTextBox(langCode,true, String.join(RegistrationConstants.SLASH, labels)));
			...
			default:
				...
				getRegistrationDTo().getSelectedLanguagesByApplicant().forEach(langcode -> {
							labels.add(this.uiFieldDTO.getLabel().get(langcode));});
				vBox.getChildren().add(createTextBox(langCode,false, String.join(RegistrationConstants.SLASH, labels)));
		...
		fieldTitle.setText(String.join(RegistrationConstants.SLASH, labels));
	...
	private HBox createTextBox(String langCode, boolean isSimpleType, String label) {
		HBox textFieldHBox = new HBox();
		TextField textField = getTextField(uiFieldDTO.getId() + langCode, label);
		...
		ValidationsCust.putIntoLabelMap(uiFieldDTO.getId() + langCode, uiFieldDTO.getLabel().get(langCode));
	...
	private TextField getTextField(String id, String label) {
		...
		if(uiFieldDTO.getId().equalsIgnoreCase("phone")) {
			//textField.setPromptText("e.g. 256700000000");
			textField.setPromptText("e.g. 0700000000");
		} else {
		...
		if (data instanceof String) {
			...
			for (SimpleDto simpleDto : list) {
				...
				if (textField != null) {
					/*
					if(uiFieldDTO.getId().equalsIgnoreCase("phone")) {
						String phone = simpleDto.getValue();
						if(phone.startsWith("0") && phone.length() == 10) {
							simpleDto.setValue("256" + phone.substring(1));
						}
					}
					 */
					textField.setText(simpleDto.getValue());
					LOGGER.info("Aiko Filling text field {}", simpleDto.getValue());
				}
			}
