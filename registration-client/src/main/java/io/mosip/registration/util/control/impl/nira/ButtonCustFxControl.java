...
public class ButtonCustFxControl extends FxControl {

	/**
	 * Instance of {@link Logger}
	 */
	private static final Logger LOGGER = AppConfig.getLogger(ButtonCustFxControl.class);
	...
	public void selectAndSet(Object data) {
		...
		if (data instanceof List) {
			...
			selectedNode = hbox.getChildren().stream()
					.filter(node1 -> {
						LOGGER.info("Button select: id - {}, list empty - {}, list value - {}, node id - {}", uiFieldDTO.getId(), list.isEmpty(), list.get(0).getValue(), node1.getId());
						return node1.getId() != null && node1.getId().equals(uiFieldDTO.getId() + (list.isEmpty() ? null : list.get(0).getValue()));
					})
					.findFirst();
		}
		else {
			selectedNode = hbox.getChildren().stream()
					.filter(node1 -> node1.getId() != null && node1.getId().equals(uiFieldDTO.getId()+(String)data))
					.findFirst();
		}
