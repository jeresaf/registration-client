...
public class DocumentCustFxControl extends FxControl {

	/**
	 * Instance of {@link Logger}
	 */
	private static final Logger LOGGER = AppConfig.getLogger(DocumentCustFxControl.class);
	...
	private VBox createDocRef(String id) {
		...
		getRegistrationDTo().getSelectedLanguagesByApplicant().forEach(lCode -> {
			...
			ResourceBundle rb = ApplicationContext.getBundle(lCode, RegistrationConstants.LABELS);
			labels.add(rb.getString(RegistrationConstants.REF_NUMBER));
		});

		String titleText = String.join(RegistrationConstants.SLASH, labels);
		ResourceBundle rb = ApplicationContext.getBundle(getRegistrationDTo().getSelectedLanguagesByApplicant().get(0), RegistrationConstants.LABELS);
	...
	public void selectAndSet(Object data) {
		...
		if (comboBox != null) {
			...
			LOGGER.info("Document Count: {}, ComboBox ID: {}", (long) comboBox.getItems().size(), comboBox.getId());
			comboBox.getItems().forEach(doc -> LOGGER.info("Doc Code: {}", doc.getCode()));
			...
			if(selected.isPresent()) {
				...
				LOGGER.error("Unable to find doc from pre-reg sync, field: {}, value: {}", uiFieldDTO.getId(), documentDto.getType());
