package io.mosip.registration.controller.nira;
...
import io.mosip.registration.util.control.impl.nira.*;
...
@Controller
public class GenericControllerCust extends BaseController {

	protected static final Logger LOGGER = AppConfig.getLogger(GenericControllerCust.class);
	...
	public static Map <Control, FxControl> otherFieldsForCheck = new HashMap<>();
	...
	private HBox getPreRegistrationFetchComponent() {
		...
		//hBox.getStyleClass().add("demoGraphicPaneContentMargin");
	...
	private void loadPreRegSync(ResponseDTO responseDTO) throws RegBaseCheckedException{
		...
		Map<String, Object> other_Attributes = successResponseDTO.getOtherAttributes();
		RegistrationDTO regDTO = (RegistrationDTO) other_Attributes.get(RegistrationConstants.REGISTRATION_DTO);
		if(regDTO != null) {
			LOGGER.info("Aiko check other attributes {}", regDTO.toString());
		} else {
			LOGGER.info("Aiko check null demographics");
			LOGGER.info("Aiko check other attributes {}", other_Attributes.toString());
		}

		HashMap<String, Object> locations = new HashMap<>(regDTO.getDemographics());

		Map<String, List<UiFieldDTO>> locationsWithHierarchy = new HashMap<>();
		...
		for (UiScreenDTO screenDTO : orderedScreens.values()) {
			for (UiFieldDTO field : screenDTO.getFields()) {
				...
				if (fxControl != null && regDTO != null) {
					switch (fxControl.getUiSchemaDTO().getType()) {
						case "biometricsType":
							break;
						case "documentType":
							fxControl.refresh();
							fxControl.selectAndSet(regDTO.getDocuments().get(field.getId()));
							break;
						default:
							try {
								if(Objects.equals(field.getSubType(), "Region") || Objects.equals(field.getSubType(), "Province") ||
										Objects.equals(field.getSubType(), "City") || Objects.equals(field.getSubType(), "Zone") ||
										Objects.equals(field.getSubType(), "Postal Code")) {
									locationsWithHierarchy.computeIfAbsent(field.getSubType(), k -> new ArrayList<>());
									locationsWithHierarchy.get(field.getSubType()).add(field);
									continue;
								}
								Object answer = regDTO.getDemographics().get(field.getId());
								...
								if(answer != null) {
									fxControl.selectAndSet(answer);
									fxControl.setData(answer);
								}
							}catch (Exception e) {
								LOGGER.info(String.format("Aiko Error: %s", e.getMessage()), e);
							}
							break;
					}
				}
		...
		//regDTO = (RegistrationDTO) responseDTO.getSuccessResponseDTO().getOtherAttributes().get(RegistrationConstants.REGISTRATION_DTO);
		LOGGER.info("Location Hierarchy: {}", locationsWithHierarchy.toString());
		LOGGER.info("Registration DTO Demographics: {}", locations.toString());

		for (UiFieldDTO field : locationsWithHierarchy.get("Region")) {
			LOGGER.info("Setting locations for district");
			setLocationAnswer(field, locations);
		}
		for (UiFieldDTO field : locationsWithHierarchy.get("Province")) {
			LOGGER.info("Setting locations for county");
			setLocationAnswer(field, locations);
		}
		for (UiFieldDTO field : locationsWithHierarchy.get("City")) {
			LOGGER.info("Setting locations for sub county");
			setLocationAnswer(field, locations);
		}
		for (UiFieldDTO field : locationsWithHierarchy.get("Zone")) {
			LOGGER.info("Setting locations for parish");
			setLocationAnswer(field, locations);
		}
		for (UiFieldDTO field : locationsWithHierarchy.get("Postal Code")) {
			LOGGER.info("Setting locations for village");
			setLocationAnswer(field, locations);
		}
	}

	/*
	private void loadRegSync(ResponseDTO responseDTO) throws RegBaseCheckedException{
		auditFactory.audit(AuditEvent.REG_DEMO_PRE_REG_DATA_FETCH, Components.REG_DEMO_DETAILS, SessionContext.userId(),
				AuditReferenceIdTypes.USER_ID.getReferenceTypeId());
	...
	*/
	private void setLocationAnswer(UiFieldDTO field, HashMap<String, Object> locations) {
		FxControl fxControl = getFxControl(field.getId());
		if (fxControl != null && locations != null) {
			try {
				Object answer = locations.get(field.getId());
				if(answer != null) {
					LOGGER.info("Location Field: {}, Location Value: {}", field.getId(), answer.toString());
					fxControl.selectAndSet(answer);
					fxControl.setData(answer);
				} else {
					LOGGER.info("Null Location Field: {}, Location Value: {}", field.getId(), locations.get(field.getId()));
				}
			}catch (Exception e) {
				LOGGER.info(String.format("Aiko Error: %s", e.getMessage()), e);
			}
	...
	private Map<String, Map<String, List<UiFieldDTO>>> getFieldsBasedOnGroupLabelAndAlignmentGroup(List<UiFieldDTO> screenFields) {
		Map<String, Map<String, List<UiFieldDTO>>> groupedScreenFields = new LinkedHashMap<>();
		...
		generalHashMap.put("eng", "generalTemplateGroup");

		screenFields.forEach( field -> {
			String group = field.getGroup() == null || field.getGroupLabel() == null || field.getGroupLabel().isEmpty() ?
					"open" : field.getGroup();
			...
			if(field.isInputRequired()) {
				if(!groupedScreenFields.containsKey(group)) {
					groupedScreenFields.put(group, new HashMap<>());
				}
				if(!groupedScreenFields.get(group).containsKey(alignmentGroup))
					groupedScreenFields.get(group).put(alignmentGroup, new LinkedList<UiFieldDTO>());
				groupedScreenFields.get(group).get(alignmentGroup).add(field);
			}
		});
		return groupedScreenFields;
	}
	...
	private void showHideErrorNotificationPrefill(String message) {
		Tooltip toolTip = new Tooltip("prefillLocations");
		toolTip.prefWidthProperty().bind(notification.widthProperty());
		toolTip.setWrapText(true);
		notification.setTooltip(toolTip);
	...
	public void populateScreens() throws Exception {
		...
		for(UiScreenDTO screenDTO : orderedScreens.values()) {
			count++;
			LOGGER.info("Aiko Check Screen {} : {} ------------ {}", count, screenDTO.getName(), screenDTO.toString());
			//Map<String, List<UiFieldDTO>> screenFieldGroups = getFieldsBasedOnAlignmentGroup(screenDTO.getFields());
			Map<String, Map<String, List<UiFieldDTO>>> screenFieldGroups = getFieldsBasedOnGroupLabelAndAlignmentGroup(screenDTO.getFields());
			...
			Map<String, FxControl> fields = new HashMap<>();
			for(Entry<String, Map<String, List<UiFieldDTO>>> mainGroupEntry : screenFieldGroups.entrySet()) {
				LOGGER.info("Aiko screen group {}", mainGroupEntry.getKey());
				GridPane groupGridPane = new GridPane();
				groupGridPane.setId(mainGroupEntry.getKey());
				groupGridPane.setMinWidth(20.0);
				groupGridPane.setMinHeight(20.0);
				groupGridPane.getStyleClass().add("sectionDemographic");
				int groupRowIndex = 0;
				Label label = null;
				if(!Objects.equals(mainGroupEntry.getKey(), "open")) {
					VBox ageVBox = new VBox();
					ageVBox.setPrefWidth(390);

					label = new Label();
					label.setId("Group " + mainGroupEntry.getKey() + " Label");
					label.setText("");
				...
				for(Entry<String, List<UiFieldDTO>> groupEntry : mainGroupEntry.getValue().entrySet()) {
					FlowPane groupFlowPane = new FlowPane();
					groupFlowPane.prefWidthProperty().bind(groupGridPane.widthProperty());
					groupFlowPane.setHgap(20);
					groupFlowPane.setVgap(20);
					List<String> prefillButtons =  new ArrayList<>();
					for (UiFieldDTO fieldDTO : groupEntry.getValue()) {
						try {
							...
							if(label != null && label.getText().isEmpty()) {
								label.setText(fieldDTO.getGroupLabel().get(getRegistrationDTOFromSession().getSelectedLanguagesByApplicant().get(0)));
								otherFieldsForCheck.put(label, fxControl);
							}
							if(prefillButtons.isEmpty() && fieldDTO.getLocationHierarchy() != null && !fieldDTO.getLocationHierarchy().isEmpty()) {
								prefillButtons.addAll(fieldDTO.getLocationHierarchy());
								String locationPrefill = fieldDTO.getLocationHierarchy().get(0);
								String[] prefillDetails = locationPrefill.split(":");
								String[] prefillLocations = prefillDetails[2].split(",");
								LOGGER.info("Prefill Locations - {}", Arrays.toString(prefillLocations));

								CheckBox checkBox = new CheckBox("Same as " + prefillDetails[0]);
								checkBox.setId("button" + prefillDetails[1] + groupRowIndex);
								checkBox.setWrapText(true);
								checkBox.setMinWidth(200);
								checkBox.setMaxWidth(790);
								checkBox.selectedProperty().addListener((options, oldValue, newValue) -> {
									if(checkBox.isSelected()) {
										showHideErrorNotificationPrefill("");
										boolean dataValid = true;
										String message = "";
										if(prefillDetails.length>3) {
											String[] checkCountry = prefillDetails[3].split(";");
											FxControl countryField = fields.get(checkCountry[0]);
											Object currentCountry = countryField.getData();
											if(currentCountry == null) {
												checkBox.setSelected(false);
												dataValid = false;
												message = "No country selected";
												LOGGER.info("{}", message);
											} else if (currentCountry instanceof List) {

												List<SimpleDto> list = (List<SimpleDto>) currentCountry;
												LOGGER.info("Check Country Data is List: {}", list.get(0).getValue());
												if(Objects.equals(checkCountry[1], list.get(0).getValue())) {
													dataValid = true;
													message = "";
													LOGGER.info("County selected is valid");
												} else {
													dataValid = false;
													message = "Country selected is not " + checkCountry[1];
													LOGGER.info("{}", message);
													checkBox.setSelected(false);
												}

											} else if (currentCountry instanceof String) {

												LOGGER.info("Check Country Data is String: {}", currentCountry);
												if(((String) currentCountry).equals(checkCountry[1])) {
													dataValid = true;
													message = "";
													LOGGER.info("County selected is valid");
												} else {
										...
										if(dataValid) {
											for (String locationName : prefillLocations) {
												String[] fillAndPrefill = locationName.split(";");

												LOGGER.info("Prefill Locations Split - {} - {}", fillAndPrefill[1], fillAndPrefill[0]);

												FxControl fieldForPrefill = fields.get(fillAndPrefill[1]);
												FxControl fieldWithData = fields.get(fillAndPrefill[0]);

												Object fieldWithDataObject = fieldWithData.getData();
												if(fieldWithDataObject == null) {
													LOGGER.info("Empty {}", fillAndPrefill[1]);
													showHideErrorNotificationPrefill("Some fields to be used for prefill are empty");
													checkBox.setSelected(false);
													break;
												} else if (fieldWithDataObject instanceof List) {
													List<SimpleDto> list = (List<SimpleDto>) fieldWithDataObject;
													LOGGER.info("Field With Data Object is List: {}", list.get(0).getValue());
												...
												} else if (fieldWithDataObject instanceof String) {
													LOGGER.info("Field With Data Object is string: {}", fieldWithDataObject);
													if(((String) fieldWithDataObject).isEmpty()) {
														LOGGER.info("Empty {}", fillAndPrefill[1]);
														checkBox.setSelected(false);
														showHideErrorNotificationPrefill("Some fields to be used for prefill are empty");
														break;
												...
												LOGGER.info("Data - {}", fieldWithData.getData().toString());

												fieldForPrefill.selectAndSet(fieldWithData.getData());
												//fieldForPrefill.setData(fieldWithData.getData());
											}
										} else {
											checkBox.setSelected(false);
											showHideErrorNotificationPrefill(message);
										}
									} else {
										for (String locationName : prefillLocations) {
											String[] fillAndPrefill = locationName.split(";");
											LOGGER.info("Prefill Locations Split - {} - {}", fillAndPrefill[1], fillAndPrefill[0]);
											FxControl fieldForPrefill = fields.get(fillAndPrefill[1]);
											fieldForPrefill.selectAndSet(null);
										}
									}
								});
								groupFlowPane.getChildren().add(checkBox);
								otherFieldsForCheck.put(checkBox, fxControl);
							}
							fields.put(fieldDTO.getId(), fxControl);
					...
					/*
					for(String locationPrefill : prefillButtons) {
						String[] prefillDetails = locationPrefill.split(":");
						Button button = new Button();
						button.setId("button" + prefillDetails[1] + groupRowIndex);
						button.getStyleClass().add("demoGraphicPaneContentButton");
						button.setText("Prefill From " + prefillDetails[0]);
						...
						button.setOnAction(event -> {
							...
							if(prefillDetails.length>3) {
								...
								FxControl countryField = fields.get(checkCountry[0]);
								Object currentCountry = countryField.getData();
								if (currentCountry instanceof List) {
							...
							if(dataValid) {
								for (String locationName : prefillLocations) {
									...
									Object fieldWithDataObject = fieldWithData.getData();
									if (fieldWithDataObject instanceof List) {
										List<SimpleDto> list = (List<SimpleDto>) fieldWithDataObject;
										LOGGER.info("Field With Data Object is List: {}", list.get(0).getValue());
										if(list.isEmpty() || list.get(0).getValue().isEmpty()) {
											LOGGER.info("Empty {}", fillAndPrefill[1]);
						...
						groupFlowPane.getChildren().add(button);
					}
					*/
					groupGridPane.add(groupFlowPane, 0, groupRowIndex++);
				}
				gridPane.add(groupGridPane, 0, rowIndex++);
			}
		...
		if (uiFieldDTO.getControlType() != null) {
			switch (uiFieldDTO.getControlType()) {
				case CONTROLTYPE_TEXTFIELD:
					if(uiFieldDTO.getId().equalsIgnoreCase("addressLine2")) {
						uiFieldDTO.getLabel().put("eng", "Address Line 2");
					}
					if(uiFieldDTO.getId().equalsIgnoreCase("addressLine1")) {
						uiFieldDTO.getLabel().put("eng", "Address Line 1");
					}
					if(uiFieldDTO.getId().equalsIgnoreCase("addressLine3")) {
						uiFieldDTO.getLabel().put("eng", "Address Line 3");
					}
					//^[2]{1}[5]{1}[6]{1}([0-9]{9})$
					//^[+]*([0-9]{1})([0-9]{9})$
					/*
					if(uiFieldDTO.getId().equalsIgnoreCase("phone")) {
						List<Validator> validators = uiFieldDTO.getValidators();
						validators.forEach(validator -> {
							if(validator.getType().equalsIgnoreCase("regex")) {
								validator.setValidator("^256([0-9]{9})$");
							}
						});
					}
					 */
					if(uiFieldDTO.getId() .equalsIgnoreCase("fullName")) {
						fxControl = new FullNameTextFieldFxControl().build(uiFieldDTO);
					} else {
						fxControl = new TextFieldCustFxControl().build(uiFieldDTO);
					}
					break;
				...
				case CONTROLTYPE_BUTTON:
					fxControl =  new ButtonCustFxControl().build(uiFieldDTO);
					break;

				case CONTROLTYPE_CHECKBOX:
					/*
					if(uiFieldDTO.getId() .equalsIgnoreCase("consent")) {
						fxControl = new RadioButtonFxControl().build(uiFieldDTO);
					} else {
						fxControl = new CheckBoxFxControl().build(uiFieldDTO);
					}
					*/
					fxControl = new CheckBoxCustFxControl().build(uiFieldDTO);
					break;

				case CONTROLTYPE_DOB:
					fxControl =  new DOBCustFxControl().build(uiFieldDTO);
					break;

				case CONTROLTYPE_DOB_AGE:
					fxControl =  new DOBAgeCustFxControl().build(uiFieldDTO);
					break;

				case CONTROLTYPE_DOCUMENTS:
					fxControl =  new DocumentCustFxControl().build(uiFieldDTO);
					break;

				case CONTROLTYPE_DROPDOWN:
					fxControl = new DropDownCustFxControl().build(uiFieldDTO);
					break;
	...
	public void refreshFields() {
		...
		//for (Entry<Control, FxControl> entry : otherFieldsForCheck.entrySet()) {
		//	if(entry.getValue())
		//}
	}
