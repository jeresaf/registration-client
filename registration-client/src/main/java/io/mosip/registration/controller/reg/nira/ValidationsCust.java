 ...
 */
@Component
public class ValidationsCust extends BaseController {

	/**
	 * Instance of {@link Logger}
	 */
	private static final Logger LOGGER = AppConfig.getLogger(ValidationsCust.class);
	...
	private boolean languageSpecificValidation(Pane parentPane, TextField node, String id, ResourceBundle messageBundle,
		...
		try {
			...
			UiFieldDTO uiFieldDTO = GenericControllerCust.getFxControlMap().get(id).getUiSchemaDTO();
	...
	private boolean checkForValidValue(Pane parentPane, TextField node, String fieldId, String value,
		...
		if (RegistrationConstants.AGE_DATE.equals(uiFieldDTO.getControlType())) {
			boolean isValid = dateValidation.validateDate(parentPane, uiFieldDTO.getId()) && dateValidation.validateAge(parentPane, uiFieldDTO.getId());
	...
	private Validator getRegex(String fieldId, String regexType, String langCode) {
		UiFieldDTO uiFieldDTO = GenericControllerCust.getFxControlMap().get(fieldId).getUiSchemaDTO();
