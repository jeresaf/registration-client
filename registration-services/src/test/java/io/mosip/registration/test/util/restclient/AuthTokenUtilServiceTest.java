...
public class AuthTokenUtilServiceTest {
	...
	public void fetchAuthTokenTest2() throws RegBaseCheckedException {
		...
		Mockito.when(serviceDelegateUtil.prepareURLByHostName(Mockito.anyString())).thenReturn("https://api-internal.techno-associates.live/v1/syncdata/authenticate/useridpwd");
		Mockito.when(environment.getProperty("auth_by_password.service.url")).thenReturn("https://api-internal.techno-associates.live/v1/syncdata/authenticate/useridpwd");
	...
	public void sendOtpTest() throws ExhaustedRetryException, Throwable {
		...
		Mockito.when(clientCryptoService.signData(Mockito.any())).thenReturn("test".getBytes());
		Mockito.when(CryptoUtil.encodeToURLSafeBase64(Mockito.any())).thenReturn("test");
		Mockito.when(serviceDelegateUtil.prepareURLByHostName(Mockito.anyString())).thenReturn("https://api-internal.techno-associates.live/v1/syncdata/authenticate/sendotp");
		Mockito.when(environment.getProperty("auth_by_otp.service.url")).thenReturn("https://api-internal.techno-associates.live/v1/syncdata/authenticate/sendotp");
		Map<String, Object> responseMap = new LinkedHashMap<>();
		Map<String, Object> respBody = new LinkedHashMap<>();
